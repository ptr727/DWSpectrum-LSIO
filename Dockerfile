...
# After: Blank lines
    && sed -i '/^    su/d; /--chuid/d' ./vms_server/opt/${appname}/mediaserver/bin/mediaserver \
...
# Replace the LSIO abc usernames with the server app names
    && usermod -l ${appname} abc \
    && groupmod -n ${appname} abc \
    && sed -i "s/abc/\${appname}/g" /etc/cont-init.d/10-adduser \
